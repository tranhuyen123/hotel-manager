...
namespace QLKhachSan.GUI.Controls
{
    public partial class QuanLyThietBi : Form
    {
        ...
        private void button3_Click(object sender, EventArgs e)
        {
              ...
              // var maphong = dataGridViewTB.Rows[Index].Cells[3].Value.ToString();

            //var dv = db.THIETBI_SD.Where(x => x.MaTB== maTB).FirstOrDefault();
            //var dv1 = db.THIETBI_SD.Where(x => x.MaPhong == maphong).FirstOrDefault();

         
            var maTB = dataGridViewTB.Rows[Index].Cells[0].Value.ToString();
           
            var dv = db.THIETBIs.Where(x => x.MaTB== maTB).FirstOrDefault();
            var lstSddv = db.THIETBI_SD.Where(x => x.MaTB== maTB).ToList();
